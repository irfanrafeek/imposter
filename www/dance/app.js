...
(() => {
  ...
  // so this file (and auth.js) can never double-init.
  // ============================================================
  if (db) {
  ...
  const chat = mountChat({
    ...
    opener: 'Found a bug, have an idea, or want more categories, or games? I’d love to hear from you.',
    me: 'user',
  ...
  // file binds them to analytics/music). Only game-specific trackers
  // remain below.
  ...
  function trackSongMiss(query) {
    ...
    const geo = peekGeo();
    const cc = (geo && geo.cc) || 'ZZ'; // ZZ = country not yet resolved
    bumpAnalytics({
