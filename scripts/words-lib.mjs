...
// is exactly the kind of logic that breaks silently. It lives here so
// scripts/words.test.mjs can hold it still. See that file for the cases.
//
// fold(), norm() and tokens() moved on again, to www/shared/fold.js, when
// the word game's clue board needed the same question answered in the
// browser (#244). They are re-exported from here so every caller and every
// test still reads them from one place; the rules themselves now have one
// home rather than two.
import { fold, norm, tokens } from '../www/shared/fold.js';
